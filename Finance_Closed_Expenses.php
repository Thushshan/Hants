...
if(isset($_POST["movepending"])){
	...
	$sql_del = "update expenses set status='$status',last_updated_by='$verified_by',last_updated_date='$verified_date' where id='$id'";
						if($conn->query($sql_del) == true){
							  		set_success_msg("<strong>Success!</strong> EXP".$id." successfully moved to pending!");
									header("Location: Finance_Closed_Expenses.php");
							  }
						  else{
				...
				<div class="col-md-10">
					<nav>
						<div class="nav nav-tabs" id="nav-tab" role="tablist">
                            ...
                            <a class="nav-item nav-link active"><strong> Closed Expenses 
                            <?php if($row_close>0){
					...
					<div class="tab-content">
						<div class="tab-pane mt-4 show active">
							<form method="post" action="Finance_Closed_Expenses.php">
								<div class="row">
									<div class="col-md-12">
										<div class="input-group mb-3">
											...
											<div class="input-group-append">
                                                    ...
                                                    <a class="btn btn-secondary" href="Finance_Closed_Expenses.php"> Refresh </a>
											</div>
										</div>
									</div>
								</div>
                                            ...
                                            <?php
												...
												if(isset($_POST["search"])){
													...
													if(empty($s_text)){
														...
														$sql_delete = "select * from expenses where status='closed'";
													}else{
														$sql_delete = "select * from expenses where ".$search." like '%".$s_text."%' and status='closed'";
													}
                                            			...
                                            			<tbody>
															<tr class="<?php echo $bg_color; ?>">
                                                    			...
                                                    			<th scope="row" class="<?php echo $row["status"]; ?>"><?php echo $row["status"]; ?></th>
                                                   				<th scope="row">
                                             					<form action="Finance_Closed_Expenses.php" method="post" onSubmit="return confirm('WARNING!\n\n1. Accidentally closeing of records cannot backup from the system.\n2. There is no way to undo this action.\n\nDo you still really want to closed EXP<?php echo $row["id"]; ?>?');">
