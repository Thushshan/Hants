...
<?php
			...
			if(isset($_POST["pay_enroll"]))
			{
				...
				$sql_cou = "update library_members set renewal_paid_status = 'closed' where user_id ='$paid_by'";
				...
				values ('Library Management','Membership Renewal Fees','$library_member_id','$paymethod','$amount','$paid_by','$added_by','$added_date','pending')";
				
						  if($conn->query($sql_ins) == true && $conn->query($sql_cou) == true){
							  		set_success_msg("<strong>Success!</strong> Library Renewal Membership fee has been successfully inserted!");
									header("Location: payments-renew-library-membership.php");
							  }
						  else{
									...
									<div class="col-md-12">
										<table class="table table-bordered">
											<thead>
														...
														if(isset($_POST['search']))
															{
																...
																$sql  = "select * from course_enrollments WHERE
																		...
																		$sql = "SELECT * FROM library_members where renewal_paid_status = 'pending'";
															}
												...
												<tbody>
												<form action="payments-renew-library-membership.php" method= "post">
												<tr>
																<th scope="row">
																...
																?>
																<td>
                                                                <input type="number" name="amount" min="0" required="required" placeholder=" Membership Renewal Fee " class="form-control" />
                                                                </td>
                                                                <td>
                                                                <input type="text" name="pay_method" placeholder="Payment Method" class="form-control" required>
                                                                </td>
																<td class="<?php echo $row['renewal_paid_status']; ?>"><center><?php echo $row['renewal_paid_status']; ?></center></td>
													<td>
																	
																	<input type="submit" value=" Renew Library Membership " class="btn btn-success btn-block" name="pay_enroll">
													</td>
												</tr>
                                                </form>
											</tbody>
